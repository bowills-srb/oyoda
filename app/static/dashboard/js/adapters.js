...
export function normalizeMessageFeed(payload = {}) {
  ...
  const items = safeArray(payload.items).map((item) => ({
    ...
    propertyBindingCandidates: safeArray(item.property_binding_candidates),
    ...
    propertyId: safeString(item.property_id),
...
export function renderInlineError(title, detail, retryId) {
  return `
    <div class="empty" style="padding:32px 20px">
      ...
      <div class="empty-sub">${escapeHtml(detail)}</div>
      ${retryId ? `<button class="btn btn-sm" id="${escapeHtml(retryId)}" style="margin-top:12px">Retry</button>` : ''}
    </div>
  `;
}
