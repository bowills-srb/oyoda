/**
 * api.js — Typed fetch wrappers for every /app/api/* endpoint.
 *
 * Sections import from this file rather than calling fetch() directly.
 * When a backend contract changes, you update one place.
 */
...
export const api = {
  ...
  // ── Dashboard summary (sidebar badges + overview cards) ────────────────
  ...
  messagingObservability: (days = 7) => request(`/app/api/messaging-observability?days=${encodeURIComponent(days)}`),
  messagingEvents: (limit = 12) => request(`/app/api/messaging-events?limit=${encodeURIComponent(limit)}`),
  // Compare-mode review surface for Audit. Same backend endpoint, different
  // shape: response has composer_rows + composer_count populated and the
  // legacy "events" array empty. See operator_dashboard_api.messaging_events.
  messagingComposerEvents: (limit = 20) => request(`/app/api/messaging-events?composer_only=true&limit=${encodeURIComponent(limit)}`),
  onboarding: {
    status: () => request('/app/api/onboarding-status'),
  },

  // ── Operator-scoped data ────────────────────────────────────────────────
  properties:  () => request('/app/api/properties'),
  propertyLinks: {
    suggest: (query, limit = 5) => request(`/app/api/property-links/suggest?query=${encodeURIComponent(query)}&limit=${encodeURIComponent(limit)}`),
    create:  (body)             => request('/app/api/property-links', { method: 'POST', body }),
    updateProfile: (propertyCode, body) => request(`/app/api/properties/${encodeURIComponent(propertyCode)}/identity`, { method: 'PATCH', body }),
    reviews: (status = 'pending') => request(`/app/api/property-links/reviews?status=${encodeURIComponent(status)}`),
    approveReview: (reviewId, body) => request(`/app/api/property-links/reviews/${encodeURIComponent(reviewId)}/approve`, { method: 'POST', body }),
    rejectReview: (reviewId) => request(`/app/api/property-links/reviews/${encodeURIComponent(reviewId)}/reject`, { method: 'POST' }),
  },
  propertyIdentity: {
  ...
  propertyImports: {
    ...
    saveProfiles: (profiles) => request('/app/api/properties/import-profiles', { method: 'PATCH', body: { profiles } }),
    replay: (ingestEventId) => request(`/app/api/properties/ingest-events/${encodeURIComponent(ingestEventId)}/replay`, { method: 'POST' }),
  },
  propertyDocuments: {
    upload: ({
      file,
  ...
  propertyAssets: {
    ...
    upsert:    (propertyCode, body) => request(`/app/api/properties/${encodeURIComponent(propertyCode)}/assets`, { method: 'POST', body }),
  },
  insights:    () => request('/app/api/insights'),
  propertiesAutonomy: {
    list:      ()               => request('/app/api/properties/autonomy'),
    update:    (id, body)       => request(`/app/api/properties/${encodeURIComponent(id)}/autonomy`, { method: 'PATCH', body }),
  ...
  team: {
    ...
    updateScopes: (id, scopes)   => request(`/app/api/team/${encodeURIComponent(id)}/scopes`, { method: 'PUT', body: { scopes } }),
  },
  portfolios: {
    list:       ()               => request('/app/api/portfolios'),
    create:     (body)           => request('/app/api/portfolios', { method: 'POST', body }),
    updateProperties: (id, property_external_ids) => request(`/app/api/portfolios/${encodeURIComponent(id)}/properties`, { method: 'PUT', body: { property_external_ids } }),
  },
  gateway: {
  ...
  // ── Knowledge Base ──────────────────────────────────────────────────────
  kb: {
    list:    (property_id)       => request('/app/api/kb' + (property_id ? `?property_id=${encodeURIComponent(property_id)}` : '')),
  ...
  sessions: {
    ...
    sendUpdate: (id, body)       => request(`/app/api/sessions/${encodeURIComponent(id)}/send-update`, { method: 'POST', body }),
    actions: (id)                => request(`/app/api/sessions/${encodeURIComponent(id)}/actions`),
    executeAction: (id, actionType) => request(`/app/api/sessions/${encodeURIComponent(id)}/actions/${encodeURIComponent(actionType)}/execute`, { method: 'POST' }),
  },

  // ── Pre-booking / unified messages ─────────────────────────────────────
  messages: {
    list:    (params)            => {
      const qs = params
  ...
  escalations: {
    ...
    assign:  (id, assigned_to)   => request(`/app/api/escalations/${encodeURIComponent(id)}/assign`, { method: 'POST', body: { assigned_to } }),
    coordination: (id, body)     => request(`/app/api/escalations/${encodeURIComponent(id)}/coordination`, { method: 'POST', body }),
    resolve: (id, notes)         => request(`/app/api/escalations/${encodeURIComponent(id)}/resolve`, { method: 'POST', body: { notes } }),
    dispatchVendor: (id, body)   => request(`/app/api/escalations/${encodeURIComponent(id)}/dispatch-vendor`, { method: 'POST', body }),
    ...
    executeAction: (id, actionType) => request(`/app/api/escalations/${encodeURIComponent(id)}/actions/${encodeURIComponent(actionType)}/execute`, { method: 'POST' }),
  },
  ...
  settings: {
    ...
    updateAlertContact: (id, body) => request(`/app/api/settings/alert-routing/${encodeURIComponent(id)}`, { method: 'PATCH', body }),
    markAlertOOO: (id, body)     => request(`/app/api/settings/alert-routing/${encodeURIComponent(id)}/ooo`, { method: 'POST', body }),
    markAlertAvailable: (id)     => request(`/app/api/settings/alert-routing/${encodeURIComponent(id)}/available`, { method: 'POST' }),
    deleteAlertContact: (id)     => request(`/app/api/settings/alert-routing/${encodeURIComponent(id)}`, { method: 'DELETE' }),
  },

  workOrders: {
    updateStatus: (id, body)     => request(`/app/api/work-orders/${encodeURIComponent(id)}/status`, { method: 'POST', body }),
  },
  ...
  admin: {
    ...
    llmUsage: {
      summary:    (window = '24h', tenantId = '') => request(`/app/api/admin/llm-usage/summary?window=${encodeURIComponent(window)}${tenantId ? `&tenant_id=${encodeURIComponent(tenantId)}` : ''}`),
      byService:  (window = '24h', tenantId = '') => request(`/app/api/admin/llm-usage/by-service?window=${encodeURIComponent(window)}${tenantId ? `&tenant_id=${encodeURIComponent(tenantId)}` : ''}`),
      byProvider: (window = '24h', tenantId = '') => request(`/app/api/admin/llm-usage/by-provider?window=${encodeURIComponent(window)}${tenantId ? `&tenant_id=${encodeURIComponent(tenantId)}` : ''}`),
      byTenant:   (window = '24h') => request(`/app/api/admin/llm-usage/by-tenant?window=${encodeURIComponent(window)}`),
      ...
      burnRate:   (tenantId = '') => request(`/app/api/admin/llm-usage/burn-rate${tenantId ? `?tenant_id=${encodeURIComponent(tenantId)}` : ''}`),
    },
  },

  // ── Raw audit / market / health (root-level, not /app/api) ─────────────
  raw: {
